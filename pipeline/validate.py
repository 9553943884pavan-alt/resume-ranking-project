"""
validate.py — Submission spec validator (verbatim from Resume_Project.ipynb).
"""
...
def validate_submission(csv_path: str) -> list:
    errors = []
    path = Path(csv_path)
    ...
    try:
        with open(path, "r", encoding="utf-8", newline="") as f:
            ...
            # Check that first 3 columns are candidate_id, rank, score (reasoning is optional but we include it)
            required_core = ["candidate_id", "rank", "score"]
            if header[:3] != required_core:
                errors.append(
                    "Row 1 (header) must start with:\n"
                    f"  {','.join(required_core)}\n"
                    f"Found:\n"
                    f"  {','.join(header[:3])}"
                )
    ...
    for i, cells in enumerate(data_rows):
        ...
        if len(cells) < 3:
            errors.append(
                f"Row {row_num}: expected at least 3 columns "
                f"(candidate_id, rank, score), got {len(cells)}."
            )
            continue
